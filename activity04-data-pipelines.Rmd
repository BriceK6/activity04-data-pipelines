...
**Response**: Flase brought up red print, error "Registered S3 methods overwritten by 'dbplyr':
  method         from
  print.tbl_lazy     
  print.tbl_sql "
  
  with conflicts in the dplyr package
...
**Response**: I found no difference with message equal to FALSE, TRUE and while turned off. "just deleted ,message = "
...
**Response**: You would use a sorting algorithm like merge or insertion sort. I'm not sure about Excel, you might be able to easily organize by column in ascending order. 
...
```{r}
highest_prop_woman <- college_recent_grads %>%
...
**Response**: All social services involving taking care of and nurturing others.
...
stem_low_salaries <- college_recent_grads %>%
  ...
  filter(median < 35000 & college_recent_grads$major_category %in% c("Biology & Life Science","Computers & Mathematics", "Engineering", "Physical Sciences"))
```

Discuss your output as it relates to the research question.

**Response**: for some reason I couldn't figure out how to use stem_categories in the filter argument, so I just put it directly in there. 
Biology, zoology, and ecology are logical lower income fields. Lower industrial demand and a higher supply due to interest in plants in animals. As for biology, maybe its too generalized and the higher paying careers involve a more specialized domain knowledge.  
